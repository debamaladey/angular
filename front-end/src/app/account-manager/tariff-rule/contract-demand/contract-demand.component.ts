...
@Component({
	selector: 'app-contract-demand',
	templateUrl: './contract-demand.component.html',
	styleUrls: ['./contract-demand.component.scss']
  })
  export class ContractDemandComponent implements OnInit {
	contractDemandFormGroup: FormGroup;
	currentUser;
	returnUrl: string;
	errorMsg;
	succMsg;
	...
	ngOnInit() {
		...
		this.tariffRule = 'contract-demand' + '/' + this.customerId + '/' + this.tariffConfigId;
		...
		this.contractDemandFormGroup = this.fb.group({
		...
		this.tariffRuleService.getTariffRule(this.customerId, this.tariffConfigId).subscribe(data => {
			if (data.list.length > 0) {
				this.contractDemandFormGroup.patchValue({
	...
	addTariffRuleFields() {
		for (let i = 0; i < this.tariffVariable.length; i++) {
			const ri = this.fb.group({
				...
				trl4_contract: ['', [Validators.required, CustomValidators.numeric]],
			});
			const riArray = this.contractDemandFormGroup.get('rule_items') as FormArray;
			riArray.push(ri);
		}
	}
			...
			for (let j = 0; j < data.length; j++) {
				...
				if (data[j].tcvn_id == this.tariffVariable[i].tcvn_id) {					
					const ri = this.fb.group({
						...
						trl4_contract: [data[j].trl4_contract, [Validators.required, CustomValidators.numeric]],
					});
					const riArray = this.contractDemandFormGroup.get('rule_items') as FormArray;
					riArray.push(ri);
				}
	...
	onSubmit() {
		...
		if (this.contractDemandFormGroup.invalid) {
			this.errorMsg = 'Please fill all required fields.';
			return;
		} else {
			this.tariffRuleService.setContractDemand(this.contractDemandFormGroup.value).subscribe(
				data => {
					this.returnUrl = 'account-manager/tariff-rule/template/' + this.customerId;
					this.succMsg = 'Contract demand tariff rule set successfully';
