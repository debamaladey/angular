...
export class LoadDetailsComponent implements OnInit {
	loadDetailsFormGroup: FormGroup;
	currentUser;
	returnUrl: string;
	errorMsg;
	succMsg;
	...
	ngOnInit() {
		...
		this.tariffRule = 'load-details' + '/' + this.customerId + '/' + this.tariffConfigId;
		...
		this.loadDetailsFormGroup = this.fb.group({
		...
		this.tariffRuleService.getTariffRule(this.customerId, this.tariffConfigId).subscribe(data => {
			if (data.list.length > 0) {
				this.loadDetailsFormGroup.patchValue({
	...
	addTariffRuleFields() {
		for (let i = 0; i < this.tariffVariable.length; i++) {
			...
			const riArray = this.loadDetailsFormGroup.get('rule_items') as FormArray;
			riArray.push(ri);
		}
	}
	...
	onSubmit() {
		...
		if (this.loadDetailsFormGroup.invalid) {
			this.errorMsg = 'Please fill all required fields.';
			return;
		} else {
			this.tariffRuleService.setLoadDetails(this.loadDetailsFormGroup.value).subscribe(
				data => {
					this.returnUrl = 'account-manager/tariff-rule/template/' + this.customerId;
					this.succMsg = 'Load details tariff rule set successfully';
